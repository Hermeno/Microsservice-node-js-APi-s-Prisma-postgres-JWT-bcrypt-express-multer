...
// Substitua "historico_beneficios" por cada nome de modelo

// CREATE
router.post('/', async (req, res) => {
  try {
    const data = req.body;
    const created = await prisma.historico_beneficios.create({ data });
...
router.delete('/:id', async (req, res) => {
  try {
    await prisma.historico_beneficios.delete({ where: { id: parseInt(req.params.id) } });
    res.status(204).send();
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
